...
at::Tensor runTorchArange_opset11(
  ...
  switch (dtype) {
    ...
    default: {
      std::cerr << "Warning: Constant folding - ONNX Range type: " << dtype
...
c10::optional<at::Tensor> runTorchBackendForOnnx(
  ...
  if (node->kind() == onnx::Slice) {
    ...
    auto ones = at::ones(inputTensorValues[0].sizes(), inputTensorValues[0].dtype());
    ...
    std::vector<int64_t> size_data = {total_size};
    auto f =
        at::from_blob(size_data.data(), {1}, at::kLong)
            .to(at::kCPU);
    // Need copy here
    at::Tensor f_copy = at::empty({1}, options);
    f_copy.copy_(f);
    return c10::optional<at::Tensor>(at::squeeze(f_copy, 0));
  } else {
    return c10::nullopt;
  }
}
