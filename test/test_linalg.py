...
class TestLinalg(TestCase):
    ...
    # See: https://github.com/pytorch/pytorch/issues/51303
    @skipCUDAIfRocm
    def test_det_autograd(self, device, dtype):
        ...
        # size (size[-1], 1) and (1, size[-1])
        def make_singular_matrix_batch(size):
            assert size[-1] == size[-2]

            batch = torch.empty(size, device=device, dtype=dtype)

            matrix_size = size[-2:]
            view_size = ((1,) + size) if len(size) == 1 else ((int(np.prod(size[:-2])),) + matrix_size)
            ...
            for matrix_idx in range(batch_view.size(0)):
                # TODO: linalg_det_backward can sometimes produce NaNs if any
                # singular values are 0. This is likely due to its dependence
                # upon svd_backward. Therefore, we must avoid testing such
                # matrices. Once lu_backward is available, linalg_det_backward
                # can be updated to use it so that wo no longer have this
                # problem: https://github.com/pytorch/pytorch/issues/53364
                svd_has_zero = True

                # Note: When calculating the determinant, floating point errors
                # can be introduced such that the calculated determinant is
                # a very small number, rather than exactly zero. We use random
                # integers cast to float to drastically decrease the chance of
                # this happening and then double check that the determinant is
                # exactly zero. If determinant is nonzero, we regenerate
                # a limited number of times until it is zero.
                det_is_nonzero = True
                ...
                while det_is_nonzero or svd_has_zero:
                    num_tries += 1
                    self.assertTrue(
                        num_tries < max_tries,
                    ...
                    if dtype.is_complex:
                        ...
                        # generating a matrix with a singular value equal to 0
                    ...
                    matrix = a @ b
                    det_is_nonzero = matrix.det().ne(0)
                    svd_has_zero = matrix.svd()[1].eq(0).any()

                assert matrix is not None
                batch_view[matrix_idx] = matrix

            return batch.requires_grad_(True)

        def make_nonsingular_matrix_batch(size):
            batch = None
            num_tries = 0
            max_tries = 10
            det_is_zero = True
            while det_is_zero:
                self.assertTrue(
                    ...
                    f' after trying {max_tries} times')
                batch = make_tensor(size, device, dtype, low=None, high=None, requires_grad=True)
                det_is_zero = batch.det().eq(0).any()
            return batch

        # Nonsingular matrices
        for input_size in [(4, 4), (2, 4, 4), (3, 2, 4, 4)]:
            a = make_nonsingular_matrix_batch(input_size)
            gradcheck(torch.linalg.det, [a])
            gradgradcheck(torch.linalg.det, [a], atol=1e-3, check_batched_grad=False)

        # Singular matrices
        for input_size in [(4, 4), (2, 4, 4), (3, 2, 4, 4)]:
            a = make_singular_matrix_batch(input_size)
            gradcheck(torch.linalg.det, [a])

            # TODO: double grad for singular inputs is unstable for both
            # complex and real types. This would most likely be fixed by
            # replacing the svd_backward call with lu_backward, when it is
            # available: https://github.com/pytorch/pytorch/issues/53364
            # gradgradcheck(torch.linalg.det, [a], atol=1e-3, check_batched_grad=False)

        # Mixed singular and nonsingular
        a = torch.stack([
            make_nonsingular_matrix_batch((4, 4)),
            make_singular_matrix_batch((4, 4))])
        gradcheck(torch.linalg.det, [a])
        ...
        a = torch.stack([
            make_nonsingular_matrix_batch((2, 4, 4)),
            make_singular_matrix_batch((2, 4, 4))])
        gradcheck(torch.linalg.det, [a])
