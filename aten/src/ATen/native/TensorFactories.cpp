...
Tensor cov(const Tensor& self,
  ...
  const Tensor& fweights = fweightsOpt.value_or(Tensor());
  const Tensor& aweights = aweightsOpt.value_or(Tensor());
  auto bias = biasOpt.value_or(false);
  auto ddof = ddofOpt.value_or((bias == false) ? 1 : 0);
  auto rowvar = rowvarOpt.value_or(true);
  ...
  TORCH_CHECK(self.dim() <= 2,
              fn_name,
              ": expected Input1 to have two or fewer dimensions, but got ", self.dim(), "!");
  TORCH_CHECK(other.dim() <= 2,
                fn_name,
                ": expected Input2 to have two or fewer dimensions, but got ", other.dim(), "!");
  if(other.defined()) {
    TORCH_CHECK(self.device().type() == other.device().type(),
                fn_name,
                ": device type of input2 (", other.device().type(), ") must match device type of Input1 (",
                self.device().type(), ").");
  }
  if(fweights.defined()) {
    TORCH_CHECK(self.device().type() == fweights.device().type(),
                fn_name,
                ": device type of fweights (", other.device().type(), ") must match device type of Input1 (",
                self.device().type(), ").");
  }
  if(aweights.defined()) {
    TORCH_CHECK(self.device().type() == aweights.device().type(),
                fn_name,
                ": device type of aweights (", other.device().type(), ") must match device type of Input1 (",
                self.device().type(), ").");
  }

  // Main tensor (X) init and concat
  Tensor X, y;
  ...
  if(w.size(0) != 0) {
    ...
    avg = at::mean(X,1);
    w_sum = at::full(X.size(0), X.size(1));
  }

  if(w.size(0) == 0) {
    fact = at::tensor(X.size(1) - ddof, self.options());
  }
  else if(ddof == 0) {
    fact = w_sum;
  }
  else if(aweights.size(0) == 0) {
    fact = w_sum - ddof;
  }
  else {
